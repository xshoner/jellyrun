...
function hitPirateFire(f){const p=s.p,unprotected=!f.hit&&!f.destroyed&&p.inv<=0&&s.buff[1]<=0&&s.buff[5]<=0&&s.buff[6]<=0,hadRevive=s.revive;damage(f,40);if(unprotected&&s.mode==='running'&&!(hadRevive&&!s.revive)){p.burn=5;p.burnTick=1;toast('화상! 5초간 매초 에너지 −2%')}}
...
function drawBurn(){const p=s.p;if(p.burn<=0||s.mode==='dying'||s.mode==='over')return;ctx.save();ctx.globalAlpha=.75;ctx.shadowColor='#ff591c';ctx.shadowBlur=18;for(let i=0;i<7;i++){const x=p.x+(i-3)*13*p.scale,y=GROUND-p.y-8,h=(28+14*Math.sin(s.t*13+i*2))*p.scale;ctx.fillStyle=i%2?'#ffe36a':'#ff6928';ctx.beginPath();ctx.moveTo(x-10*p.scale,y);ctx.quadraticCurveTo(x-16*p.scale,y-h*.6,x+Math.sin(s.t*9+i)*9,y-h);ctx.quadraticCurveTo(x+18*p.scale,y-h*.4,x+9*p.scale,y);ctx.fill()}ctx.restore()}
