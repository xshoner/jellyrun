...
const effectDescriptions=['','15초간 보호막 · 피해 1회 방어!','에너지 30% 즉시 회복!','쓰러지면 에너지 10%로 부활!','10초간 주변 젤리 자동 수집!','8초간 무적! 이동 속도 3배','10초간 무적! 몸집 3배 · 속도 1.5배'];
...
function burst(x,y,color,n=20){n=Math.min(cheapEffects?Math.ceil(n/2):n,Math.max(0,(cheapEffects?120:240)-s.fx.length));for(let i=0;i<n;i++)s.fx.push({x,y,vx:rand(-160,160),vy:rand(-230,60),life:rand(.35,.8),max:.8,color,r:rand(2,6)})}
...
function returnToApp(fromPop=false){if(s.mode==='running')pause();unlockLandscape(fromPop);hud()}
...
function hasSpecialMonster(){return s.goblinFollowup!==null||s.pirates.length>0||s.iceMonsters.length>0||s.batters.length>0||s.projectiles.length>0||s.baseballs.length>0||s.goblins.length>0}
...
function drawBurn(front=false){
 ...
 if(!front){
  ...
  ctx.shadowColor='#ff4b16';ctx.shadowBlur=shadowRadius(22*k);
  ...
  ctx.shadowColor='#ff961f';ctx.shadowBlur=shadowRadius(10*k);
...
function updateBombs(dt){
 ...
 for(const b of s.bombs){b.x-=s.speed*dt;if((b.phase==='armed'||b.phase==='igniting')&&b.x-b.w/2<W&&!soundPlayer?.channels?.has('bombAlarm'))playSfx('bombAlarm',{channel:'bombAlarm',loop:true});b.flash+=dt*bombFlashRate(b);b.age+=dt;
  if(b.phase==='armed'&&b.x-s.p.x<=Math.max(190,s.speed*BOMB_TRIGGER_TIME)){b.phase='igniting';b.age=0;tone(180,.12)}
  else if(b.phase==='igniting'&&b.age>=.16){b.phase='exploding';b.age=0;burst(b.x,GROUND-75,'#ff9c53',32);soundPlayer?.stopChannel('bombAlarm');playSfx('bomb',{channel:'bombExplosion'})}
 ...
 if(!s.bombs.some(b=>!b.destroyed&&(b.phase==='armed'||b.phase==='igniting')))soundPlayer?.stopChannel('bombAlarm');
...
function update(dt){if(s.mode==='running'&&s.bonus){updateBonus(dt);return}if(s.mode==='dying'){s.death-=dt;s.shake=0;advanceJump(s.p,dt);if(!s.p.falling)s.p.y=Math.max(0,s.p.y);s.p.anim+=dt;updateFx(dt);if(s.death<=0)finish();return}if(s.mode!=='running')return;s.t+=dt;const meters=Math.floor((s.t+1e-8)*10);s.score+=meters-s.meters;s.meters=meters;const stage=1+Math.floor(s.t/45);if(stage!==s.stage){s.stage=stage;toast(`STAGE ${String(stage).padStart(2,'0')} · 속도가 빨라집니다!`);playSfx('stage',{channel:'stage',limit:1.5})}const p=s.p;p.panic=Math.max(0,p.panic-dt);if(p.panic<1e-8)p.panic=0;
 ...
 s.speed=BASE_SPEED*Math.pow(1.13,s.stage-1)*(s.buff[5]>0?3:1)*(s.buff[6]>0?1.5:1);s.distance+=s.speed*dt;s.bg+=s.speed*dt*.42;
 p.scale+=( (s.buff[6]>0?3:1)-p.scale)*Math.min(1,dt*9);if(s.giantEffectPending&&p.scale>=2.9){s.giantEffectPending=false;playSfx('giantEffect',{channel:'giantEffect'})}if(p.frozen<=0&&p.panic<=0)p.anim+=dt*(s.buff[5]>0?1.8:1)*(s.buff[6]>0?1.5:1);for(const k of ['hit','inv','blink','land'])p[k]=Math.max(0,p[k]-dt);s.shake=Math.max(0,s.shake-dt);s.heal=Math.max(0,s.heal-dt);s.resurrection=Math.max(0,s.resurrection-dt);
 ...
 for(let id=1;id<=6;id++){let spawn=false;if(s.t>=s.nextItems[id]){s.nextItems[id]+=schedule[id].interval;spawn=Math.random()<Math.max(0,schedule[id].chance-.03*(s.stage-1))}if(s.t>=s.nextGuarantees[id]){s.nextGuarantees[id]+=schedule[id].guarantee;spawn=true}if(spawn)spawnItem(id)}
...
function tunnelTiles(o){
 ...
 if(!o.tiles||o.tiles.length!==o.tileCount)o.tiles=Array.from({length:o.tileCount},(_,index)=>({index,x:0,w:0}));
 for(const tile of o.tiles){tile.x=o.x+tile.index*width;tile.w=width}
 return o.tiles;
}
function updatePickups(dt){
 ...
 for(const q of s.pickups){
  q.x-=s.speed*dt;
  if((q.type==='jelly'||q.type==='bonus')&&s.buff[4]>0){const dx=p.x-q.x,dy=GROUND-p.y-55*p.scale-q.y,r=CELL*p.scale*3;if(dx*dx+dy*dy<r*r){const f=1-Math.exp(-dt*12);q.x+=dx*f+s.speed*dt*.8;q.y+=dy*f}}
  if(q.taken||!overlap(pb,{x:q.x-q.w/2,y:q.y-q.h/2,w:q.w,h:q.h}))continue;
  q.taken=true;
  if(q.type==='item'){acquire(q.id);continue}
  const points=q.type==='bonus'?q.points:[0,10,25,100][q.id]*(q.large?1.3:1);
...
function spawnGoblin(){
 ...
 s.goblinFollowup=Math.random()<.4?1:null;
 addGoblin();return true;
}
function addGoblin(){
...
function updateGoblins(dt){
 if(s.goblinFollowup!==null){s.goblinFollowup-=dt;if(s.goblinFollowup<=1e-8){s.goblinFollowup=null;addGoblin()}}
...
function drawPortal(){
 ...
 ctx.save();ctx.shadowColor='#d394ff';ctx.shadowBlur=shadowRadius(18+Math.sin(portal.age*6)*8);
...
function activateBonus(){
 ...
 s.energy=saved.energy;s.meters=saved.meters;s.t=saved.t;s.stage=saved.stage;s.p.x=saved.p.x;s.speed=saved.speed*1.5;
 s.bonus={phase:'active',elapsed:0,nextWave:0,patternBag:[],lastPattern:null,saved};
 spawnBonusPattern(s.p.x+100);
}
function spawnBonusPattern(origin){
 const b=s.bonus;
 if(!b.patternBag.length){b.patternBag=['arc','wave','stairs','diamond','double','zigzag','heart','columns'];for(let i=b.patternBag.length-1;i>0;i--){const j=Math.floor(rand(0,i+1));[b.patternBag[i],b.patternBag[j]]=[b.patternBag[j],b.patternBag[i]]}if(b.patternBag[0]===b.lastPattern)b.patternBag.reverse()}
 const type=b.patternBag.shift();b.lastPattern=type;
 const spacing=rand(58,78),height=rand(160,240),count=12;
 for(let i=0;i<count;i++){const t=i/(count-1),x=origin+i*spacing;let ys;
 if(type==='arc')ys=[60+Math.sin(t*Math.PI)*height];
 else if(type==='wave')ys=[170+Math.sin(t*Math.PI*2)*110];
 else if(type==='stairs')ys=[60+(i%4)*65];
 else if(type==='diamond')ys=[170+(1-Math.abs(t*2-1))*110,170-(1-Math.abs(t*2-1))*110];
 else if(type==='double')ys=[65,210];
 else if(type==='zigzag')ys=[60+(1-Math.abs((t*4%2)-1))*height];
 else if(type==='heart'){const a=t*Math.PI*2;bonusJelly(origin+350+240*Math.sin(a)**3,GROUND-190-9*(13*Math.cos(a)-5*Math.cos(2*a)-2*Math.cos(3*a)-Math.cos(4*a)),100);continue}
 else ys=i%2?[65]:[65,135,205,275];
 for(const y of ys)bonusJelly(x,GROUND-y,100);
 }
 b.nextWave=b.elapsed+(origin+(count-1)*spacing+100-(W+60))/s.speed;
}
...
function updateBonus(dt){
 ...
 s.distance+=s.speed*activeDt;s.bg+=s.speed*activeDt*.42;s.p.anim+=activeDt*1.5;
 for(const k of ['hit','inv','blink','land'])s.p[k]=Math.max(0,s.p[k]-activeDt);
 updateGround(activeDt);
 if(b.elapsed>=b.nextWave)spawnBonusPattern(W+60);
...
function updateFx(dt){let live=0;for(const f of s.fx){f.x+=f.vx*dt;f.y+=f.vy*dt;if(!f.text)f.vy+=380*dt;f.life-=dt;if(f.life>0)s.fx[live++]=f}s.fx.length=live}
...
function hud(){const gamePause=$('gamePause');gamePause.hidden=!['running','paused'].includes(s.mode);gamePause.textContent=s.mode==='paused'?'▶ 플레이':'Ⅱ 일시정지';gamePause.setAttribute?.('aria-label',s.mode==='paused'?'게임 계속하기':'게임 일시정지');gamePause.setAttribute?.('aria-pressed',String(s.mode==='paused'));$('exitGameFullscreen').hidden=!fullscreenSession||!['running','paused'].includes(s.mode);$('bonusTimer').hidden=!s.bonus;setText('bonusTimer',s.bonus?'BONUS · '+Math.ceil(Math.max(0,15-s.bonus.elapsed))+'s':'');setText('distance',s.meters.toLocaleString()+' m');setText('score',String(s.score).padStart(6,'0'));setText('energyText',s.energy+'%');$('energy').style.transform='scaleX('+s.energy/100+')';$('energy').style.background=s.energy<=25?'#ff776e':'#d6ff63';setText('stage','STAGE '+String(s.stage).padStart(2,'0'));setText('time',formatTime(s.t));setText('speed','SPEED ×'+(s.speed/BASE_SPEED).toFixed(2));$('stageProgress').style.transform='scaleX('+(s.t%45/45)+')';setText('buffs',[...Object.entries(s.buff).filter(([,v])=>v>0).map(([id,v])=>names[id]+' '+Math.ceil(v)+'s'),...(s.revive?['부활 준비 ✓']:[]),...(s.p.frozen>0?['빙결 '+s.p.frozen.toFixed(1)+'s']:[]),...(s.p.burn>0?['화상 '+Math.ceil(s.p.burn)+'s']:[]),...(s.p.panic>0?['경직 '+s.p.panic.toFixed(1)+'s']:[])].join(' · ')||'젤리 '+s.jellies+'개 · 몬스터 피해 −25%')}
function sprite(im,sx,sy,sw,sh,x,y,w,h){const kx=im.sourceScaleX||1,ky=im.sourceScaleY||1;ctx.drawImage(im,sx*kx,sy*ky,sw*kx,sh*ky,x,y,w,h)}
...
// Only the narrow feathered edge needs an extra surface (about 4 MiB total).
const backgroundEdges=new Map();
...
function character(){const p=s.p,k=p.scale,sl=sliding(),dead=s.mode==='dying'||s.mode==='over';let row=0,col=Math.floor(p.anim*11)%4;if(dead){row=4;col=0}else if(p.panic>0){row=3;col=0}else if(p.hit>0){row=3;col=0}else if(p.jumps){row=1;col=p.vy>180?0:p.vy>-260?1:2}else if(sl){row=2;col=0}if(!dead&&p.frozen>0&&p.frozenPose){row=p.frozenPose.row;col=p.frozenPose.col}
 ...
 const x=p.x,y=GROUND-p.y;ctx.save();ctx.imageSmoothingEnabled=false;// Falling below the road has no ground shadow; negative radii throw and stop RAF.
 if(!p.falling&&p.y>=0){ctx.fillStyle='#06141450';ctx.beginPath();ctx.ellipse(x,GROUND+3,40*k/(1+p.y/350),8*k,0,0,Math.PI*2);ctx.fill()}
 if(s.buff[5]>0){for(let i=cheapEffects?2:4;i>0;i--){ctx.globalAlpha=.09*(5-i);sprite(im,col*sw,run?0:row*sh,sw,sh,x-CELL*k/2-i*22,y-CELL*k,CELL*k,CELL*k)}ctx.globalAlpha=1;ctx.shadowColor='#fff5ac';ctx.shadowBlur=shadowRadius(32);ctx.fillStyle='#ffefaa40';ctx.beginPath();ctx.ellipse(x-20,y-55*k,85*k,54*k,0,0,7);ctx.fill();ctx.shadowBlur=0}
...
function drawPirates(){
 ...
 for(const f of s.projectiles){ctx.save();ctx.translate(f.x,f.y);ctx.scale(f.scale||1,f.scale||1);ctx.shadowColor='#ff651f';ctx.shadowBlur=shadowRadius(22);const g=ctx.createLinearGradient(-22,0,65,0);g.addColorStop(0,'#fff9bf');g.addColorStop(.4,'#ff9d20');g.addColorStop(1,'#ff391000');ctx.fillStyle=g;ctx.beginPath();ctx.moveTo(-26,0);ctx.quadraticCurveTo(-5,-28,62+Math.sin(f.age*35)*10,-12);ctx.lineTo(37,0);ctx.lineTo(68,13);ctx.quadraticCurveTo(-4,28,-26,0);ctx.fill();ctx.fillStyle='#ffffd9';ctx.beginPath();ctx.ellipse(-5,0,15,8,0,0,7);ctx.fill();ctx.restore()}
}
function drawIce(){
 ...
 for(const m of s.iceMonsters){
  ...
  if(m.phase==='laser'){
   ...
   ctx.globalAlpha=power;ctx.shadowColor='#00aaff';ctx.shadowBlur=shadowRadius(35);ctx.fillStyle='#1688ff55';ctx.fillRect(0,beamY-thickness*.9,muzzle,thickness*1.8);
  ...
  const count=iceCountdown(m);if(count){ctx.save();ctx.textAlign='center';ctx.shadowColor='#20bfff';ctx.shadowBlur=shadowRadius(18);ctx.fillStyle='#e6ffff';ctx.font='900 64px "Malgun Gothic", sans-serif';ctx.fillText(count,W/2,245);ctx.shadowBlur=0;ctx.font='bold 18px "Malgun Gothic", sans-serif';ctx.fillText('점프로 피한다',W/2,280);ctx.restore()}
 }
}
...
function drawFrozen(){const p=s.p;if(p.frozen<=0)return;const x=p.x,y=GROUND-p.y,k=p.scale,w=100*k,h=119*k;
 ctx.save();ctx.translate(x,y);const ice=ctx.createLinearGradient(-w/2,-h,w/2,0);ice.addColorStop(0,'#d7ffffbb');ice.addColorStop(.4,'#1ab6ff50');ice.addColorStop(1,'#87eaff99');ctx.fillStyle=ice;ctx.strokeStyle='#c5ffff';ctx.lineWidth=3;ctx.shadowColor='#42cfff';ctx.shadowBlur=shadowRadius(18);
...
function drawBombs(){const frames=[{sx:0,w:650},{sx:650,w:680},{sx:1330,w:842}];for(const b of s.bombs){const index=b.phase==='armed'?0:b.phase==='igniting'?1:2,f=frames[index],exploding=index===2,size=exploding?BOMB_BASE*BOMB_SCALE:100;ctx.save();ctx.imageSmoothingEnabled=false;
 if(b.phase==='spent')ctx.globalAlpha=Math.max(0,1-b.age/.3);else if(!exploding&&Math.floor(b.flash*2)%2===0){if(cheapEffects)ctx.globalAlpha=.6;ctx.filter=cheapEffects?'none':'sepia(1) saturate(9) hue-rotate(315deg) brightness(1.3)';ctx.shadowColor='#ff3030';ctx.shadowBlur=shadowRadius(20)}
...
function drawBaseball(){const frames=[{sx:0,w:575,anchor:315},{sx:575,w:875,anchor:600},{sx:1450,w:722,anchor:380}];
 ...
 for(const b of s.baseballs){ctx.save();ctx.translate(b.x,b.y);ctx.scale(b.scale||1,b.scale||1);ctx.strokeStyle='#ffd68190';ctx.lineWidth=13;ctx.beginPath();ctx.moveTo(20,0);ctx.lineTo(100,0);ctx.stroke();ctx.rotate(b.age*12);ctx.shadowColor='#ffd16e';ctx.shadowBlur=shadowRadius(22);ctx.fillStyle='#fffaf0';ctx.beginPath();ctx.arc(0,0,30,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.strokeStyle='#d74343';ctx.lineWidth=3;for(const side of [-1,1]){ctx.beginPath();ctx.moveTo(side*14,-25);ctx.quadraticCurveTo(side*2,0,side*14,25);ctx.stroke();for(let j=-16;j<=16;j+=8){ctx.beginPath();ctx.moveTo(side*9-4,j-2);ctx.lineTo(side*9+4,j+2);ctx.stroke()}}ctx.restore()}
}
...
function drawNotices(){if(s.mode==='over'||s.mode==='dying')return;const p=s.p;ctx.save();if(s.speech)bubble(s.speech.text,p.x,Math.max(150,GROUND-p.y-CELL*p.scale-28),'#faffed',s.speech.scream);if(s.itemNotice){const n=s.itemNotice,age=3-(n.until-s.t);ctx.globalAlpha=Math.min(1,(n.until-s.t)*3,age*6);bubble(n.text,clamp(p.x+200,270,W-280),Math.max(215,GROUND-p.y-CELL*p.scale-100-(s.speech?45:0)),colors[n.id])}ctx.restore()}
// Magnet/bonus bursts reuse the same score glyphs instead of rasterizing them per frame.
const floatingLabels=new Map();
function drawFloating(f){
 ctx.font='bold 20px "Malgun Gothic", sans-serif';ctx.textAlign='center';
 if(!document.createElement){ctx.fillText(f.text,f.x,f.y);return}
 const key=f.text+'|'+f.color;let label=floatingLabels.get(key);
 if(!label){
  label=document.createElement('canvas');const c=label.getContext('2d');
  if(!c?.fillText){ctx.fillText(f.text,f.x,f.y);return}
  label.width=Math.ceil(ctx.measureText(f.text).width)+8;label.height=36;
  c.font=ctx.font;c.textAlign='center';c.fillStyle=f.color;c.fillText(f.text,label.width/2,26);
  if(floatingLabels.size>=32)floatingLabels.delete(floatingLabels.keys().next().value);
  floatingLabels.set(key,label);
 }
 ctx.drawImage(label,f.x-label.width/2,f.y-26);
}
function draw(){configureRenderer();ctx.clearRect(0,0,W,H);if(!ready)return;ctx.save();if(s.mode==='running'&&s.shake>0)ctx.translate(rand(-5,5),rand(-4,4));bg();for(const q of s.pickups){if(q.x+q.w/2+40<0||q.x-q.w/2-40>W)continue;const bob=Math.sin(s.t*4+q.phase)*3;ctx.save();if(q.type==='item'){ctx.shadowColor=colors[q.id];ctx.shadowBlur=shadowRadius(18);ring(q.x,q.y+bob,31+Math.sin(s.t*4)*2,colors[q.id]+'99',2)}let im=assets[q.type+q.id];ctx.drawImage(im,q.x-q.w/2,q.y-q.h/2+bob,q.w,q.h);ctx.restore()}for(const o of s.obstacles)monster(o);drawBombs();drawPirates();drawIce();drawBaseball();drawGoblins();drawPortal();drawBurn(false);if(s.mode!=='ready')character();drawBurn(true);drawFrozen();drawPanic();drawNotices();for(const f of s.fx){ctx.globalAlpha=clamp(f.life/f.max,0,1);ctx.fillStyle=f.color;if(f.text){drawFloating(f)}else{ctx.beginPath();ctx.arc(f.x,f.y,f.r,0,7);ctx.fill()}}ctx.globalAlpha=1;drawBonusTransition();ctx.restore()}
let nextDraw=0,nextHud=0,lastDrawMode='';
function frame(now){
 const elapsed=last?now-last:0,workStart=globalThis.performance?.now?.()||0;
